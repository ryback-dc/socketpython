...
class ThreadedServer(object):
    ...
    def listen(self):
        ...
        s.connect((atghost, atgport))
        print "[ATG][CONNECT] >> connected on %s Port %s" % (atghost, atgport)

        while True:
            client, address = self.sock.accept()
            threading.Thread(target = self.listenToClient,args = (client,address, s)).start()
            #threading.Thread(target = self.listenToAtg,args = (client,address, s)).start()
